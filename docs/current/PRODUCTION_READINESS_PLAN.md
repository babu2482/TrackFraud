...
However, it currently feels like a loose, disconnected mess — inconsistent UI, broken components, duplicated data sources, missing pagination, in-memory caches that won't scale, no auth on admin, etc.
...
**Key Decisions Made During Planning Review:**
- **Category data:** Delete `FraudCategory` model entirely. Use string slugs with CHECK constraints. One source of truth: `lib/categories.ts`.
- **Deployment:** Self-hosted (Docker Compose on VPS) over Vercel. Keeps PostgreSQL, Redis, MeiliSearch co-located and costs less.
- **Job queue:** BullMQ (Redis-based). Already have Redis, battle-tested, no vendor lock-in.
- **Admin auth:** Simple cookie secret. Internal tooling, single admin.
- **FraudMap:** Fix webpack first. Fall back to static SVG if it takes more than 2 hours.
- **MeiliSearch:** Keep for now (typo tolerance matters). If search doesn't work by end of Phase 3, fall back to Postgres FTS temporarily.
...
| Duplicate category data (DB `FraudCategory` model to delete) | 🔴 High | Schema cleanup needed |
...
| Rate limiting duplicated in API routes | 🟡 Medium | Search API reimplements its own rate limiter |
| No legal disclaimers or takedown process | 🟡 Medium | Liability risk for fraud data platform |
...
|---------|----------------|
| Categories | `lib/categories.ts` only — delete DB `FraudCategory` model, use string slugs |
...
1. Add entry to `lib/categories.ts` (add slug to CHECK constraints in schema)
...
**That's it.** No UI code changes needed. The dynamic category page at `app/[category]/page.tsx` handles rendering. The `categoryId` field on `CanonicalEntity` is a plain String (no FK relation to manage).
...
#### 1.7 Consolidate Rate Limiting
- **Problem:** `app/api/search/route.ts` implements its own in-memory rate limiter, separate from `lib/rate-limiter.ts`. Every API route should use the same rate limiter.
- **Fix:**
  - Remove the inline rate limiter from `app/api/search/route.ts`
  - Use `lib/rate-limiter.ts` (Redis-backed with in-memory fallback)
  - Audit all other API routes for duplicated rate limiting
  - Add rate limiting to any routes missing it
...
- [ ] All API routes use `lib/rate-limiter.ts` (no duplicated rate limiters)
- [ ] All 353 unit tests still pass
- [ ] All 27 E2E tests still pass

---

### Phase 2: Data Layer Optimization (Days 4-7)

**Goal:** Database performs well at scale. Queries are indexed. Data is consistent. FraudCategory model eliminated.
...
#### 2.3 Eliminate `FraudCategory` Model — Use String Slugs
- **Problem:** Categories stored both in `lib/categories.ts` AND in DB `FraudCategory` model. The sync-script approach creates ongoing operational debt.
- **Fix:** Delete `FraudCategory` entirely. Convert `categoryId` to plain `String` with CHECK constraint.

**Why:** Categories are a classification scheme (not user-generated data). They change infrequently (currently 16, growing slowly). PostgreSQL string columns are fast, and CHECK constraints enforce referential integrity at the DB level.

**What changes:**
- `CanonicalEntity.categoryId` — String, no relation
- `Tip.categoryId` — String, no relation
- `Subscriber.categoryId` — String, no relation
- `SourceSystem.categoryId` — String, no relation
- **Delete:** `FraudCategory` model, all relations

**Prisma migration:**
```prisma
// Add CHECK constraint listing valid category slugs
model CanonicalEntity {
  // ...
  categoryId String
  // No more: FraudCategory FraudCategory @relation(...)

  @@check("categoryId IN ('charities', 'corporate', 'government', 'healthcare', 'political', 'consumer', 'financial-services', 'insurance', 'cybersecurity', 'environmental', 'immigration', 'housing', 'education', 'pharmaceutical', 'energy', 'supply-chain')")
}
```

**Migration script:**
```typescript
// scripts/migrate-category-refs.ts
// Run this BEFORE dropping the FraudCategory model.
// Verifies all categoryId values in existing rows match a valid slug.
...
async function verify() {
  ...
  // Check each table
  for (const model of ['CanonicalEntity', 'Tip', 'Subscriber', 'SourceSystem']) {
    const invalid = await prisma.$queryRaw`
      SELECT "categoryId" FROM "${model}" 
      WHERE "categoryId" NOT IN (${validSlugs.join(',')})
    `;
    if (invalid.length) {
      throw new Error(`${model} has invalid categoryIds: ${JSON.stringify(invalid)}`);
    }
...
**Tradeoff acknowledged:** You lose native FK enforcement at the DB level. For a platform whose categories are a config-driven classification scheme, this is acceptable. The config file + CHECK constraint is sufficient.

**Result:** One source of truth (`lib/categories.ts`). No sync script. No mental overhead. No join overhead.

#### 2.4 Meilisearch Index Optimization
...
- **Kill Switch:** If MeiliSearch is not returning useful results by end of this phase, fall back to PostgreSQL full-text search (tsvector/GIN indexes) as interim solution. Do not block launch on search.
...
- [ ] `FraudCategory` model deleted from schema
- [ ] All `categoryId` fields converted to String with CHECK constraints
- [ ] Search index consistency verified
- [ ] Database backup/restore tested
- [ ] MeiliSearch tuned or Postgres FTS fallback documented

---

### Phase 3: UI/UX Cohesion (Days 8-12)

**Goal:** Every page looks consistent, professional, and serves the mission.

#### 3.1 Consolidate Category Pages
...
#### 3.2 Fix Legacy Dark Mode Classes
...
#### 3.3 Replace Emoji with SVG Icons Everywhere
...
#### 3.4 Standardize Loading States
...
#### 3.6 Legal Disclaimers & Takedown Process
- **Problem:** The platform aggregates and displays fraud data about real entities. No disclaimers exist.
- **Fix:**
  - Add footer disclaimer on every page: "Not legal advice. Data sourced from public records. Use at your own discretion."
  - Add `/disclaimer` page with full legal language
  - Add takedown process: `/contact/takedown` form for entities to request review of inaccurate data
  - Add source attribution badge on every data point showing which government source it came from
  - Document takedown SLA in runbooks (review within 48 hours)
...
- [ ] Takedown process documented and accessible
- [ ] E2E tests updated to match new UI

---

### Phase 4: Scalability & Performance (Days 13-16)
...
### Phase 5: Fraud Scoring & Pipeline Hardening (Days 17-20)
...
### Phase 6: Search & Discovery Enhancement (Days 21-23)
...
### Phase 7: Production Deployment & Monitoring (Days 24-30)

**Goal:** Deploy to production. Monitor everything. Respond to incidents.

#### 7.1 Production Deployment
- **Platform:** Self-hosted on VPS (Docker Compose + Nginx + systemd). Recommended: Hetzner CPX51 ($29/mo) or DigitalOcean Droplet ($48/mo).
- **Why self-hosted:** Your architecture has 3 infrastructure services (PostgreSQL, Redis, MeiliSearch) that need to be close to the app for latency. Co-locating them via Docker Compose eliminates network hops and is cheaper than managed services.
- **Database:** PostgreSQL 16 (in Docker, same host). Add automated backups (`pg_dump` cron job to S3-compatible storage).
- **Redis:** Redis 7 (in Docker, same host).
- **MeiliSearch:** MeiliSearch v1.10 (in Docker, same host).
- **Alternative:** If you need serverless, use Vercel + Upstash Redis + MeiliSearch Cloud + Supabase/Neon DB. Expect higher cost and cold starts.
...
| `scripts/migrate-category-refs.ts` | Verify categoryId values before dropping FraudCategory | 2 |
| `scripts/verify-search-consistency.ts` | Search/DB consistency check | 2 |
| `scripts/load-test.ts` | Load testing script | 4 |
| `app/error.tsx` | Root error boundary | 2 |
| `app/not-found.tsx` | Custom 404 page | 3 |
| `app/disclaimer/page.tsx` | Legal disclaimer page | 3 |
| `app/contact/takedown/page.tsx` | Takedown request form | 3 |
...
| `prisma/schema.prisma` | Add indexes, delete FraudCategory model, add CHECK constraints | 2 |
...
Also add `"new-category"` to the `@@check` constraint on `CanonicalEntity.categoryId`, `Tip.categoryId`, `Subscriber.categoryId`, and `SourceSystem.categoryId` in `schema.prisma`.
...
|------|-----------|--------|------------||
| FraudMap can't be fixed | Medium | Medium | Graceful SVG fallback map (2-hour timebox) |
| Redis unavailable | Low | Medium | In-memory fallback already exists |
| MeiliSearch not working at launch | Low | High | Fall back to PostgreSQL FTS (tsvector/GIN). Document migration path back. |
...
| Legal liability | Low | Critical | Disclaimer on all pages, takedown process, not legal advice |
| FraudCategory migration breaks queries | Low | Medium | Verify script before dropping model, feature flag the migration |
...
| 2. Data Layer | 4 days | Optimized database, FraudCategory deleted, indexes added |
| 3. UI/UX Cohesion | 5 days | Consistent, professional UI, legal disclaimers |
...
| 7. Production Deploy | 7 days | Deployed, monitored, documented, load tested |
| **Total** | **~30 days** | **Production-ready platform** |
