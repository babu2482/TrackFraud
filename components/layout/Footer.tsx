...
const DATA_SOURCES = [
  "IRS", "SEC", "FEC", "CFPB", "CMS", "OFAC", "EPA", "FDA", "HHS",
  "SAM.gov", "Congress.gov", "USASpending.gov", "ProPublica",
];

export function Footer({ variant = "full", className = "" }: FooterProps) {
  if (variant === "minimal") {
    return (
      <footer className={`border-t border-gray-200 dark:border-gray-800 py-4 px-4 ${className}`}>
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-2">
          <p className="text-xs text-gray-500 dark:text-gray-400">
            © {new Date().getFullYear()} TrackFraud · Built for public accountability
          </p>
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8">
          ...
          <div className="col-span-2 md:col-span-4 lg:col-span-1 mb-4 lg:mb-0">
            ...
            <p className="mt-2 text-sm text-gray-500 dark:text-gray-400 max-w-xs">
              Follow the money. Track fraud across charities, corporations, government, and more.
            </p>
            {/* Social / links */}
          ...
          <div>
            ...
            <ul className="space-y-2">
              {getActiveCategories().slice(6).map((cat) => (
                <li key={cat.slug}>
                  <Link
                    href={`/search?type=${cat.searchType || cat.slug}`}
                    className="text-sm text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors"
              ...
              {CATEGORIES.filter((c) => c.status === "coming_soon").slice(0, 3).map((cat) => (
                <li key={cat.slug}>
                  <span className="text-sm text-gray-400 dark:text-gray-500 cursor-default">
                    {cat.navLabel || cat.name}
                  </span>
                </li>
              ))}
        ...
        <div className="mt-10 pt-6 border-t border-gray-200 dark:border-gray-800 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-gray-400 dark:text-gray-500 text-center sm:text-left">
            © {new Date().getFullYear()} TrackFraud · Data from {DATA_SOURCES.join(", ")}, and other public records.
            <span className="block mt-1">Built for public accountability. Not legal advice.</span>
          </p>
