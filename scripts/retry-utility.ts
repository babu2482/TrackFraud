...
// Logs directory (configurable via env)
...
export class CircuitBreaker {
  ...
  getStatus(): { state: string; failures: number; willResetAt?: Date } {
    ...
    if (this.lastFailureTime) {
      status.willResetAt = new Date(
        this.lastFailureTime.getTime() + this.timeoutMs,
      );
    }
...
export class RotatingLogger {
  ...
  constructor(scriptName: string, config?: Partial<typeof DEFAULT_CONFIG>) {
    this.logFile = join(LOGS_DIR || "logs", `${scriptName}.log`);
    this.maxSizeBytes =
      (config?.MAX_LOG_FILE_SIZE_MB || DEFAULT_CONFIG.MAX_LOG_FILE_SIZE_MB) *
      1024 *
      1024;
    this.rotationCount =
      config?.LOG_ROTATION_COUNT || DEFAULT_CONFIG.LOG_ROTATION_COUNT;
...
export async function retryWithBackoff<T>(
  ...
  const initialBackoff =
    config?.initialBackoffMs || DEFAULT_CONFIG.INITIAL_BACKOFF_MS;
  const maxBackoff = config?.maxBackoffMs || DEFAULT_CONFIG.MAX_BACKOFF_MS;
  const multiplier =
    config?.backoffMultiplier || DEFAULT_CONFIG.BACKOFF_MULTIPLIER;
  ...
  while (attempts < maxRetries) {
    ...
    try {
      ...
      await logger.info(
        `${operationName} - Success after ${attempts} attempt(s)`,
      );
      ...
      await logger.error(
        `${operationName} - Attempt ${attempts} failed: ${errorMessage}`,
      );
...
export async function safeIngestionOperation<T>(
  ...
  await logger.info(
    `Max retries: ${config?.maxRetries || DEFAULT_CONFIG.MAX_RETRIES}`,
  );
  await logger.info(
    `Circuit breaker threshold: ${config?.circuitBreakerThreshold || DEFAULT_CONFIG.CIRCUIT_BREAKER_THRESHOLD}`,
  );
  ...
  if (result.success) {
    await logger.info(
      `${operationName} completed successfully in ${elapsedSeconds.toFixed(2)}s`,
    );
  } else {
    await logger.error(
      `${operationName} failed after ${elapsedSeconds.toFixed(2)}s and ${result.attempts} attempts`,
    );
  }
