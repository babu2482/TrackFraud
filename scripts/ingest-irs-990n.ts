/**
 * IRS Form 990-N (e-Postcard) ingestor.
 * Downloads and persists filings for small tax-exempt organizations.
 *
 * Usage:
 *   npm run ingest:irs-990n
 *   npm run ingest:irs-990n -- --force-download
 *   npm run ingest:irs-990n -- --max-rows 1000 --batch-size 500
 */
...
function parseArgs(argv: string[]): ParsedArgs {
  const parsed: ParsedArgs = {
    batchSize: 5000,
    downloadDir: path.resolve("data/irs/990n"),
    forceDownload: false,
  };
...
async function downloadZip(params: { downloadDir: string; forceDownload: boolean }): Promise<DownloadResult> {
  const head = await fetchHead(IRS_990N_ZIP_URL);
  ...
  const localPath = path.join(localDir, "data-download-epostcard.zip");
  ...
  const response = await fetch(IRS_990N_ZIP_URL, { signal: AbortSignal.timeout(180000) });
  if (!response.ok || !response.body) throw new Error(`990-N ZIP download failed: ${response.status}`);
...
function extractText(zipBytes: Uint8Array): string {
  ...
  const entry = Object.keys(files).find((n) => n.toLowerCase().endsWith(".txt") || n.toLowerCase().endsWith(".csv"));
  if (!entry) throw new Error("No text file found in 990-N ZIP");
  return strFromU8(files[entry]);
}

async function flushBatch(batch: Irs990NRow[], sourcePublishedAt: Date | null, stats: AggregateStats) {
  if (batch.length === 0) return;
  const result = await persistIrs990NBatch(batch, sourcePublishedAt);
...
async function main() {
  ...
  const sourceSystem = await prisma.sourceSystem.findUnique({ where: { id: IRS_990N_SOURCE_SYSTEM_ID }, select: { id: true } });
  ...
  try {
    console.log("Downloading IRS Form 990-N (e-Postcard)...");
    ...
    await prisma.rawArtifact.upsert({
      ...
      create: { sourceSystemId: IRS_990N_SOURCE_SYSTEM_ID, ingestionRunId: run.id, artifactType: "irs_990n_zip", storageProvider: "local", storageKey: download.storageKey, originalUrl: IRS_990N_ZIP_URL, checksum: download.checksum, contentType: download.contentType, byteSize: download.byteSize, sourcePublishedAt: download.sourcePublishedAt ?? undefined, fetchedAt: new Date(), parserVersion: "irs-990n-v1", status: "fetched" },
    });
    ...
    const batch: Irs990NRow[] = [];

    for (const line of lines) {
      const trimmed = line.trim();
      if (!trimmed) continue;

      // Detect and parse header on first non-empty line
      if (headerIndexes === null) {
        headerIndexes = parseIrs990NHeader(trimmed) ?? {};
      ...
      try {
        const row = parseIrs990NLine(trimmed, headerIndexes);
        if (!row) { stats.rowsSkipped++; continue; }
        stats.rowsRead++;
        batch.push(row);
      } catch (err) {
        stats.rowsFailed++;
        if (failures.length < 20) failures.push(err instanceof Error ? err.message : String(err));
      }
    ...
    await prisma.rawArtifact.update({ where: { storageKey: download.storageKey }, data: { parsedAt: new Date(), parserVersion: "irs-990n-v1", status: "parsed", errorSummary: null } });
    ...
    await prisma.sourceSystem.update({ where: { id: IRS_990N_SOURCE_SYSTEM_ID }, data: { lastSuccessfulSyncAt: status !== "failed" ? completedAt : undefined, lastError: errorSummary } });

    console.log(JSON.stringify({ runId: run.id, status, rowsRead: stats.rowsRead, rowsInserted: stats.rowsInserted, rowsUpdated: stats.rowsUpdated, rowsFailed: stats.rowsFailed, bytesDownloaded: stats.bytesDownloaded, sourceInfoUrl: IRS_990N_INFO_URL }, null, 2));
    ...
    await prisma.sourceSystem.update({ where: { id: IRS_990N_SOURCE_SYSTEM_ID }, data: { lastError: message } });
    throw error;
  } finally {
