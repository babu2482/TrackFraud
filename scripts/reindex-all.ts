...
import { INDEX_NAMES } from "../lib/search";
...
async function showIndexStatus(): Promise<void> {
  ...
  try {
    ...
    const healthInfo = await (client as any).health();
    console.log(`Meilisearch Health: ${healthInfo.status || "healthy"}`);
    ...
    try {
      const tasks = await (client as any).getTaskList();
...
async function runFullReindex(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `  Error Rate: ${((stats.failed / stats.totalProcessed) * 100).toFixed(2)}%`,
    );
...
async function runIncrementalIndex(): Promise<void> {
  ...
  if (sinceDateStr === "last-run") {
    ...
    console.error(
      "Use ISO format (2024-01-15) or relative time (6h, 12h, 24h)",
    );
    process.exit(1);
  }
  ...
  try {
    const stats = await indexNewEntities(
      sinceDate,
      100,
      INDEX_NAMES.ALL_ENTITIES,
    );
...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    case "help":
      ...
      console.log(
        '                Optional: specify date (e.g., "2024-01-15" or "6h")',
      );
      console.log(
        "  init          Initialize indexes with settings and populate\n",
      );
