...
async function runPipeline(options: PipelineOptions): Promise<void> {
  ...
  if (!options.scoreOnly) {
    ...
    if (options.category === 'charity') {
      ...
      console.log(`⚠️  Signal detection for category "${options.category}" not yet implemented`);
  ...
  if (scoreDistribution.length > 0) {
    scoreDistribution.forEach(row => {
      ...
      console.log(`  ${row.level.toUpperCase().padEnd(10)} |${bar.padEnd(40)}| ${row.count}`);
    });
  }

  // Get average score
  const avgScoreResult = await prisma.$queryRaw<Array<{ avg: number }>>`
    SELECT AVG(score) as avg 
    FROM "FraudSnapshot" 
    WHERE isCurrent = true
  `;
  
  if (avgScoreResult[0].avg) {
    console.log(`\n  Average Score: ${avgScoreResult[0].avg.toFixed(1)}/100`);
  }
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '-h':
        ...
        console.log('Usage: npx tsx scripts/run-fraud-analysis-pipeline.ts [options]\n');
        console.log('Options:');
        console.log('  --category, -c <cat>   Entity category (default: charity)');
        console.log('  --limit, -l <num>      Limit number of entities to process');
        console.log('  --detect-only          Run signal detection only, skip scoring');
        console.log('  --score-only           Skip detection, run scoring on existing signals');
        console.log('  --no-reindex           Skip search index update after scoring');
        console.log('  --help, -h             Show this help message\n');
        console.log('Examples:\n');
        console.log('  # Run full pipeline on charities (limited to 1000)');
        console.log('  npx tsx scripts/run-fraud-analysis-pipeline.ts --category charity --limit 1000\n');
        
        console.log('  # Score only (signals already detected)');
        console.log('  npx tsx scripts/run-fraud-analysis-pipeline.ts --score-only\n');
        
        console.log('  # Detection only');
        console.log('  npx tsx scripts/run-fraud-analysis-pipeline.ts --detect-only\n');
        process.exit(0);
    }
  }
