...
const FIPS_TO_STATE = new Map(US_STATES.map((s) => [s.fips, s]));
const ABBR_TO_STATE = new Map(US_STATES.map((s) => [s.abbr, s]));

export function fipsToAbbr(fips: string): string | undefined {
  return FIPS_TO_STATE.get(fips)?.abbr;
}

export function fipsToName(fips: string): string | undefined {
  return FIPS_TO_STATE.get(fips)?.name;
}

export function abbrToFips(abbr: string): string | undefined {
  return ABBR_TO_STATE.get(abbr.toUpperCase())?.fips;
}

export function abbrToName(abbr: string): string | undefined {
  return ABBR_TO_STATE.get(abbr.toUpperCase())?.name;
}
